 ...
 * @author dieup
 */
public class WarningExpD30 extends Thread{
    @Override
    public void run() {
        System.out.println("Chay thread gui thong bao gia han thanh cong goi D30");
        ...
        while (true) {
           ...
           String enable_mt_renew = vn.ctnet.confi.Ultility.getValue("enable_mt_renew");
            ...
            try {
                lsv = svDAO.getExpWarningD30();
            } catch (ClassNotFoundException | SQLException ex) {
                Logger.getLogger(WarningExpD1.class.getName()).log(Level.SEVERE, null, ex);
            }
            if (lsv != null && lsv.size() > 0) {
                for (Service sv : lsv) {
                    System.out.println("Goi thong thong bao gia han thanh cong " + sv.getPhone() + " goi D30");
